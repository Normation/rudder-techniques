...
function Invoke-CustomScriptAnalyzerRule {
        ...
        'FromCommandName' {
            try {
                ...
                if ($command.CommandType -notin 'ExternalScript', 'Function') {
                    throw New-Object -TypeName "InvalidOperationException" -ArgumentList @('The command "{0}" is not a script or function.' -f $CommandName)
                }
                $command.ScriptBlock.Ast
            } catch {
                $pscmdlet.ThrowTerminatingError(
                    New-Object -TypeName "System.Management.Automation.ErrorRecord" -ArgumentList @($_.Exception, 'InvalidCommand', 'OperationStopped', $CommandName)
                )
            }
        }
    }
    ...
    try {
        ...
        $pscmdlet.ThrowTerminatingError(
            New-Object -TypeName "System.Management.Automation.ErrorRecord" -ArgumentList @\(New-Object -TypeName "InvalidOperationException' -ArgumentList @('The name "{0}" is not a valid rule" -f $RuleName, $_.Exception),
                'InvalidRuleName',
                'OperationStopped',
                $RuleName
            )
